...
private:
    ...
    public:
        TerrainManager(Loader loader, GLuint size, Scene * scene) : size((float)size), loader(loader), scene(scene){
//            for(int i=0 ; i<10 ; i++){
//                for(int j=0 ; j<10 ; j++){
...
//            }
        ...
        void generateTerrain(PointerTexture2D texture, int vertices, float height, int textures, int posX, int posZ){
            ...
            scene -> addEntity(createEntity(t -> getModel(),
                                            Vector3f((float)posX * size, 1.0f, (float)posZ * size),
...
//            return terrains[hashVal] -> getHeight(modulo(x, size), modulo(z, size)) + 1.0f;
            return terrains[hashVal] -> getHeight((int)(x * (size / 128.0f)), (int)(z * (size / 128.0f)));
        }
