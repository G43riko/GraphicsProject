...
//Block::Block(int x, int y, int z, Chunk_old * parent, const BlockType type)
...
Block::Block(int x, int y, int z, Chunk_old * parent, const BlockIDs type)
        : //translation(Matrix4f::initTranslation((x * BLOCK_SIZE_X) << 1, (y * BLOCK_SIZE_Y) << 1, (z * BLOCK_SIZE_Z) << 1)),
//        type(BlockType::aqua),
        blockType(type),
        x(x),
...
void Block::setNeighbor(Block * block){
    ...
    if(block -> x + 1 == x || (x == 0 && block -> x == CHUNK_SIZE - 1 && block -> y == y && block -> z == z)){
        xNeighborPlus = block;
        turnOffOption(X_PLUS);
        block -> xNeighborMinus = this;
        block -> turnOffOption(X_MINUS);
    }
    else if(block -> y + 1 == y || (y == 0 && block -> y == CHUNK_SIZE - 1 && block -> x == x && block -> z == z)){
        yNeighborPlus = block;
        turnOffOption(Y_PLUS);
        block -> yNeighborMinus = this;
        block -> turnOffOption(Y_MINUS);
    }
    else if(block -> z + 1 == z || (z == 0 && block -> z == CHUNK_SIZE - 1 && block -> y == y  && block -> x == x)){
        zNeighborPlus = block;
