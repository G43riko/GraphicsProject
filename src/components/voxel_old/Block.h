...
//    Block(int x, int y, int z, Chunk_old * parent, BlockType type);
    Block(int x, int y, int z, Chunk_old * parent, const BlockIDs type);
...
//    }
    ...
    inline Chunk_old * getParent(void) const{ return parent; }
    ...
    inline Vector3f getAbsolutePos(void)const{return Vector3f(((x * BLOCK_SIZE) << 1) - ((scale.getXi() - 1) * BLOCK_SIZE),
                                                              ((y * BLOCK_SIZE) << 1) - ((scale.getYi() - 1) * BLOCK_SIZE),
                                                              ((z * BLOCK_SIZE) << 1) - ((scale.getZi() - 1) * BLOCK_SIZE)); }
private:
